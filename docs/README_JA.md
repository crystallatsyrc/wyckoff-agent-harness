...
**A株 / 香港株 / 米国株向けQuantEvoLens出来高分析エージェント -- 自然言語で話しかけると、相場を読み解く**
...
自然言語でQuantEvoLensの達人と対話しよう。彼は10個の専門ツール + 5個の汎用能力を操り、多段階推論を自律的に連鎖させ、「仕掛けるべきか否か」を教えてくれる。

Web + CLI + MCP の三系統対応、Gemini / Claude / OpenAI / DeepSeek から選択可能、GitHub Actions による完全自動化。

プロジェクトホームページ：**[crystallatsyrc.github.io/quantevolens-homepage](https://crystallatsyrc.github.io/quantevolens-homepage/)**

> リスク開示：QuantEvoLens は教育・研究・情報提供を目的としたツールです。投資助言ではなく、個人の財務状況を網羅するものでも、将来の成果を保証するものでもありません。
...
<table>
  <tr>
    ...
    <td>
      ...
      QuantEvoLens の A株 / 米国株 / 香港株の高品質マーケットデータ能力を支えてくれる TickFlow に感謝します。
    </td>
  </tr>
</table>
...
| スキル | 内蔵スラッシュコマンド（`/screen`、`/checkup`、`/report`、`/strategy`、`/backtest`）でワンタップ複合ワークフロー実行；`~/.quantevolens/skills/*.md` でユーザー拡張可能 |
...
| ローカルダッシュボード | `quantevolens dashboard` — 推奨・シグナル・保有・Agent記憶・対話ログ、ダーク/ライトテーマ、日中バイリンガル |
...
Python の検出、uv のインストール、隔離環境の作成を自動で行います。完了後 `quantevolens` で起動。
...
| `analyze_stock` | QuantEvoLens診断 / 直近 OHLCV 相場データ（mode 切替） |
...
[MCP プロトコル](https://modelcontextprotocol.io/)経由でQuantEvoLens分析機能を公開。Claude Code / Cursor / 任意のMCPクライアントから10個のツールを直接呼び出し可能。
...
登録後、Claude Code / Cursor で「000001を診断して」と聞くだけでQuantEvoLensツールが呼び出されます。

## QuantEvoLens Skills

軽量なQuantEvoLens分析機能の再利用：[`crystallatsyrc/quantevolens_skill`](https://github.com/crystallatsyrc/quantevolens_skill.git)

AIアシスタントに「QuantEvoLens視点」を素早く装着するのに最適。
