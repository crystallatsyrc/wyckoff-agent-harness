...
| 入口 | `web/apps/web/` | `quantevolens`（无子命令） | `quantevolens-mcp` |
...
| MCP（15） | `query_history`、`search_stock_by_name`、`analyze_stock`、`get_market_overview`、`screen_stocks`、`run_backtest`、`market_regime`、`quantevolens_diagnose`、`intraday_analysis`、`intraday_rescue_check`、`run_funnel_simulation`、`portfolio`、`update_portfolio`、`generate_ai_report`、`generate_strategy_decision` |
...
`mcp_server.py` — 通过 [Model Context Protocol](https://modelcontextprotocol.io) 将 QuantEvoLens 分析能力暴露给外部 AI Agent（Claude Code、Cursor 等）。

```
Claude Code / Cursor / 其他 MCP 客户端
  │
  ├─→ stdio 连接 → quantevolens-mcp 进程
  │
...
**与 CLI / Web 的关键区别**：MCP Server 不具备对话能力，它只是一个工具服务——LLM 的推理和多轮编排由外部客户端（如 Claude Code）负责，QuantEvoLens MCP 只响应单次工具调用。

安装与注册：

```bash
pip install quantevolens[mcp]
claude mcp add quantevolens -- quantevolens-mcp
```

凭证通过环境变量注入（`TUSHARE_TOKEN`、`SUPABASE_*`），或由 `_get_credential` 自动从 `~/.quantevolens/quantevolens.json` 读取。
...
| L1 | `agent_memory` 原子记忆 | 用户偏好、非显而易见的决策逻辑 | `quantevolens memory trace <id>` |
...
`agent_memory` 新增 `memory_level`、`source_ref`、`confidence`、`metadata` 字段。TUI 在保存记忆时写入 `source_ref=chat_log:<session_id>`，CLI 可用 `quantevolens memory trace <id>` 查看来源会话片段。
...
5. 超过 inline 预算（默认 8,000 字符）的工具结果由 `cli/tool_results.py` 写入 `~/.quantevolens/tool-results/*.json`，上下文只保留 `node_id`、`result_ref`、Mermaid 节点和预览；`index.jsonl` 记录节点到原文文件的映射，便于按 `node_id` 下钻。
...
`cli/dashboard.py` — `quantevolens dashboard [--port 8765]`
...
| 配置 | quantevolens.json | 模型配置（API Key 脱敏） |
...
`~/.quantevolens/agent.log` — Python `logging.FileHandler`，记录每次对话的 session_id、用户输入、耗时、token 用量。
...
| 文件 / 数据库 | 用途 |
|-------------|------|
| `quantevolens.json` | 模型配置（provider / api_key / model / base_url） |
| `session.json` | Supabase 登录态（access_token / refresh_token） |
| `agent.log` | Agent 文件日志 |
| `quantevolens.db` | SQLite 数据库（下方详述） |

### SQLite 表（quantevolens.db）
...
Supabase 不可达时静默跳过，使用本地陈旧数据。`quantevolens sync` 可手动触发。

## 五层漏斗引擎

`core/quantevolens_engine.py`，~60 可调参数（`FunnelConfig`）。
...
| **港股漏斗筛选** (`quantevolens_funnel_hk.yml`) | 周一-周五 16:35 | `market_funnel_job.py --market hk` |
| **A 股漏斗筛选 + AI 研报 + 决策** (`quantevolens_funnel.yml`) | 周日-周四 17:17 | `daily_job.py` Step2→3→4，若次日非 A 股交易日则跳过，日频写入 `theme_radar_snapshot` |
...
| **美股漏斗筛选** (`quantevolens_funnel_us.yml`) | 周二-周六 05:35 | `market_funnel_job.py --market us` |
...
| **输入预览** (`quantevolens_input_preview.yml`) | dry-run 模式查看漏斗输入 |
...
| `recommendation_tracking` | QuantEvoLens形态复盘 |
...
写入边界：GitHub Actions / server job 必须设置 `QUANTEVOLENS_WRITE_CONTEXT=server_job` 才能写共享信号、推荐、策略表。CLI 默认只能读取云端表；除持仓增删改和现金更新外，其它 CLI 结果只写本地 SQLite。
...
```bash
quantevolens                          # 启动 TUI 对话（默认）
quantevolens update                   # 升级到最新版
quantevolens auth <email> <password>  # 登录
quantevolens auth logout              # 登出
quantevolens auth status              # 查看登录状态
quantevolens model list               # 列出模型配置
quantevolens model add                # 交互式添加模型
quantevolens model set <id> ...       # 非交互式设置模型
quantevolens model rm <id>            # 删除模型
quantevolens model default <id>       # 设置默认模型
quantevolens config                   # 查看数据源配置
quantevolens config tushare <token>   # 配置 Tushare
quantevolens config tickflow <key>    # 配置 TickFlow
quantevolens portfolio list           # 查看持仓（别名 pf）
quantevolens portfolio add <code>     # 添加持仓
quantevolens portfolio rm <code>      # 删除持仓
quantevolens portfolio cash [--amount]# 查看/设置可用资金
quantevolens signal [status]          # 查看信号池
quantevolens recommend                # 查看复盘记录（别名 rec）
quantevolens dashboard [--port N]     # 启动可视化面板（别名 dash）
quantevolens sync [status]            # 手动同步 / 查看同步状态
quantevolens cleanup [--days N]       # 清理过期本地数据（默认 30 天）
quantevolens-mcp                      # 启动 MCP Server（供 Claude Code 等调用）
```
...
| Homebrew | `brew tap crystallatsyrc/quantevolens && brew install quantevolens` |
| pip | `uv pip install quantevolens` |

`install.sh`：检测 Python 3.11+ → 安装 uv → 创建 `~/.quantevolens/venv` → 安装 PyPI 包 → 符号链接到 `~/.local/bin/quantevolens`。
