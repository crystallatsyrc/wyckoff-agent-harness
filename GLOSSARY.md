...
本文档汇总了 QuantEvoLens 项目中涉及的所有金融术语、量化指标和策略概念，帮助新同学快速理解系统逻辑。
...
**QuantEvoLens** 是本项目使用的量价分析框架，核心思想：**跟踪主力资金（Composite Man / 庄家）的行为**。
...
| **Compression（压缩蓄势）** | 连续多日 ATR 收窄 + 成交量萎缩，表明供应枯竭、多空平衡即将被打破。对应 QuantEvoLens Phase B→C 的能量压缩状态，常出现在 Spring/SOS 之前 |
...
| **L4** | QuantEvoLens 触发 | 检测是否出现 Spring / SOS / LPS / EVR / Compression 等买入信号 | ~90% |
...
| **trigger_q** | QuantEvoLens 信号强度排名 | **30%** | **最高权重**，信号质量是选股第一要素 |
| **hot_bonus** | 处于当日热门板块的额外加分 | +0.02 | 板块风口的顺势加成 |
| **sector_bonus** | 板块轮动状态的加/减分 | -0.10 ~ +0.03 | 见上方板块轮动状态表 |

**设计哲学**：trigger_q 权重最高（30%），体现了项目的核心理念——**QuantEvoLens 信号质量是最重要的选股依据**，动量和板块只是辅助确认。
